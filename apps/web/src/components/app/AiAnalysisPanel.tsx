      ...
      <CardContent className="space-y-4">
        ...
        {status === "COMPLETED" ? (
          <div className="space-y-4" data-testid="ai-analysis-result">
            ...
            {analysis!.provider ? (
              <p className="text-xs text-slate-400" data-testid="ai-analysis-metadata">
                Generated by {analysis!.provider}
                {analysis!.model ? ` (${analysis!.model})` : ""}
                {analysis!.inputTokens !== null && analysis!.outputTokens !== null
                  ? ` · ${analysis!.inputTokens} + ${analysis!.outputTokens} tokens`
                  : ""}
                {analysis!.durationMs !== null ? ` · ${(analysis!.durationMs / 1000).toFixed(1)}s` : ""}
              </p>
            ) : null}
          </div>
        ) : null}
